    ...
    if (useCategories && user) {
      const fetchBudgets = async () => {
        try {
          ...
          const response = await fetch('https://budgetbuddytest-backend.onrender.com/budgets', {
            headers: {
    ...
    if (selectedBudget && user) {
      const fetchBudgetDetails = async () => {
        try {
          ...
          const response = await fetch(`https://budgetbuddytest-backend.onrender.com/budgets/${selectedBudget}`, {
            headers: {
              'Authorization': `Bearer ${user.token}`,
            },
          });
